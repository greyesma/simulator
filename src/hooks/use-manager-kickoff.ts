...
export interface UseManagerKickoffOptions {
  assessmentId: string;
...
export function useManagerKickoff({
  ...
  // Connect to Gemini Live for kickoff call
  const connect = useCallback(async () => {
    ...
    try {
      ...
      // Get ephemeral token from server for kickoff call
      const tokenResponse = await fetch("/api/kickoff/token", {
        method: "POST",
      ...
      if (!tokenResponse.ok) {
        ...
        throw new Error(data.error || "Failed to get kickoff token");
      }
      ...
      session.sendClientContent({
        turns: [
          { role: "user", parts: [{ text: "Hi, I'm here for the kickoff!" }] },
        ],
        turnComplete: true,
      });
    } catch (err) {
  ...
  }, [assessmentId, managerId, disconnect, updateConnectionState]);
